...
TEST_IMAGES = [
          ...
          "http://fc05.deviantart.net/fs70/f/2013/106/2/4/big_model_in_london__p_by_eddieswan-d61wul6.jpg",
          "http://www.disneybymark.com/wp-content/uploads/2012/11/DbMbtm04.jpg",
          "http://www.washingtonpost.com/rf/image_908w/2010-2019/WashingtonPost/2015/04/13/Style/Images/2015_MTV_Movie_Awards_-_Show-0e262.jpg",
          "http://vignette3.wikia.nocookie.net/axisandallies/images/a/a6/Big_World-NekahNets_1939.png",
          ]
...
class TwythonEndpointsTestCase(unittest.TestCase):
    ...
    @unittest.skip('skipping non-updated test')
    def test_update_image_and_destroy_status(self):
        """Test updating and deleting a status succeeds"""
        texts = [
                 "Test post with media just to get deleted:( %s" % int(time.time()),
                ]
        import urllib
        from StringIO import StringIO
        image_blob = StringIO(urllib.urlopen(TEST_IMAGES[0]).read())
        status = self.api.update_status_with_media(status=texts[0], media=image_blob)
        self.api.destroy_status(id=status['id_str'])

    def test_update_with_media_upload(self):
        import urllib
        from StringIO import StringIO
        from common.utils import twitter as twitter_util
        from common.utils import image as image_util
        image_blob = StringIO(urllib.urlopen(TEST_IMAGES[0]).read())
        media = self.api.upload_media(media=image_blob)
        media_id = media.get('media_id', None)
        self.assertIsNotNone(media_id)
        texts = [
                 "Test post with media just to get deleted:( %s" % int(time.time()),
                 "NFL Draft: Redskins's select Arkansas inside linebacker http://www.snsanalytics.com/yBrfy7 %s" % int(time.time()),
                 "How Spike's new 'Lip Sync Battle' landed its A-list hit http://www.snsanalytics.com/gp24y3 %s" % int(time.time()),
                 "Carly Fiorina Makes It Official: She Doesn’t Want Clinton to Be President. (Also She's Running, Too.) http://www.snsanalytics.com/WYxhy1",
                ]
        over_140_tweet = texts[3]
        self.assertRaises(TwythonError, lambda: self.api.update_status(status=over_140_tweet, media_ids=[media_id]))
        hacked_tweet = twitter_util.hack_tweet_with_media(over_140_tweet)
        status = self.api.update_status(status=hacked_tweet, media_ids=[media_id])
        self.api.destroy_status(id=status['id_str'])
        big_png = TEST_IMAGES[4]
        image_data = image_util.data_by_url(big_png)
        image_data = image_util.resize_data(image_data, maxdim=2000)
        media = self.api.upload_media(media=StringIO(image_data))
        media_id = media.get('media_id', None)
        self.assertIsNotNone(media_id)
        status = self.api.update_status(status=texts[0], media_ids=[media_id])
        self.api.destroy_status(id=status['id_str'])
